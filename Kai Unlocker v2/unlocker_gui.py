...
EA_APP_PATHS = [
    ...
    os.path.join(os.environ.get('ProgramFiles(x86)', 'C:\\Program Files (x86)'), 'EA Games', 'EA app', 'EA Desktop',
                 'EA Desktop'),
    os.path.join(os.environ.get('LocalAppData', os.environ.get('USERPROFILE', '')), 'Programs', 'EA app', 'EA Desktop',
                 'EA Desktop'),
]
...
class UnlockerApp(ctk.CTk):
    ...
    def run_install_process(self):
        ...
        try:
            ...
            # PASO 5: Pausa para asegurar copia
            time.sleep(2)

            # PASO 6: Configurar juegos
        ...
        try:
            ...
            for dll_path in locations:
                try:
                    ...
                    print(f"Error eliminando {dll_path}: {e}")
    ...
    def show_error_message(self, error):
        ...
        elif "No se encontró" in str(error):
            error_msg += "\n\nAsegúrate de que todos los archivos necesarios están en la misma carpeta que este programa."
    ...
    def verify_unlocker_files(self):
        ...
        base_dir = get_base_dir()
        required_files = ["setup.bat", VERSION_DLL, CONFIG_INI]
        missing_files = []
        ...
        if missing_files:
            raise FileNotFoundError(
                f"Archivos requeridos no encontrados:\n{', '.join(missing_files)}\n\n"
                f"Asegúrate de que están en: {base_dir}"
            )
    ...
    def install_local_file(self, filename, target_dir):
        ...
        if not os.path.exists(source_path):
            alt_source = os.path.join(os.path.dirname(os.path.abspath(__file__)), filename)
